...
=head1 EXTENSION API SUBROUTINES

###Add these subs to this file's SYNOPSIS!!!!!!

The subroutines in this section will ease the creation of fetchware extensions
by providing a few simple subroutines that you can use to easily create a
fetchware extension without a lot of copy and paste nonsense.
...
sub export_api {
    my %opts = @_;
    my $callers_package_name = caller;
...
#    $api_subs{$_}++ for @opts{qw(KEEP OVERRIDE)};
    ...
    die <<EOD if (grep {$api_subs{$_} == 1} keys %api_subs) == 8;
...
EOD
    ...
    for my $sub (@{$opts{KEEP}}) {
        ...
        create_config_options(IMPORT => $opts{KEEP}) unless $opts{NOIMPORT};
    }

    {
        no strict 'refs';
        # Ensure *all API subroutines are defined.
        # Clear values of %api_subs. Remember values() are aliases so changing
        # their value chages the value of that key's value in the hash.
        $_ = 0 for values %api_subs;
        for my $sub (keys %api_subs) {
            if ("$callers_package_name"->can($sub)) {
use Test::More;
note("CALLERCAN!!!!!!!!!!![$sub]");
                $api_subs{$sub}++;
            }
        }
        my @implemented_api_subs = grep {$api_subs{$_} == 1} keys %api_subs;
note explain \@implemented_api_subs;
#        my @missing_api_subs = grep {$api_subs{$_} == 0} keys %api_subs;
my @missing_api_subs;
for my $sub (keys %api_subs) {
    if (exists $api_subs{$_}) {
        push @missing_api_subs, $sub if $api_subs{$_} == 0;
    }
}
note explain \@missing_api_subs;
        die <<EOD if @missing_api_subs > 0;
App-Fetchware-Util: The fetchware extension [$callers_package_name] fails to
keep (inherit from App::Fetchware) or override all of fetchware's API
subroutines. Please implement or inherit the missing subroutines.
[@missing_api_subs]
EOD

        # Be sure to export all of the API subs.
        _add_export($_, $callers_package_name) for keys %api_subs;
    }
}


=head2 create_config_options()

    # create_config_options() *must* be called in a BEGIN block, because it
    # creates subroutines that have prototypes, and prototypes *must* be known
    # about at compile time not run time!
###BUGALERT###But isn't eval() time compile time?????????? They are eval ""'d,
#so it should be fine right making the BEGIN block unnecessary???
    BEGIN { 
        create_config_options(
            ONE => [qw(
                page_name
...
Creates configuration options of the same types make_config_sub() uses. These
are:
...
In addition to make_config_sub()'s types, create_config_options() features an
additional type:
...
that you C<'KEEP'> export_api() will automatically call create_config_options()
...
C<NOIMPORT> option, C<create_config_options(..., NOIMPORT =E<gt> 1);>, to avoid
...
create_config_options() also takes the faux hash key C<IMPORT> this hash key
...
create_config_options() creates subroutines that have prototypes, but in order
for perl to honor those prototypes perl B<must> know about them at compile-time;
therefore, that is why create_config_options() must be called inside a C<BEGIN>
block.
...
EOD
            clone($sub_name => (from => 'App::Fetchware', to => $callers_package))
                or die <<EOD;
App-Fetchware-Util: Failed to cline the specified subroutine [$sub_name] from
...
note("sub[$sub_to_export]caller[$caller]");
my $arrref = \@{"${caller}::EXPORT"};
note explain $arrref;
    }
}
